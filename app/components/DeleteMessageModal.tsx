...
import { useDeleteMessageMutation, useGetMessagesQuery } from "../services/message.service";

const DeleteMessageModal = ({ name, id }: { name: string, id: string }) => {
    const [isOpen, setIsOpen] = useState(false);

    const [deleteMesssage, { isLoading }] = useDeleteMessageMutation();
    const { refetch } = useGetMessagesQuery({ page: 1, per_page: 10 });
    ...
    const handleDelete = async () => {
        try {
            await deleteMesssage(id).unwrap();
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl dark:bg-gray-700 animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="p-6 text-center">
                            ...
                            <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                Are you sure you want to delete message from <span className="font-bold text-gray-800">"{name}"</span>?
                            </h3>
